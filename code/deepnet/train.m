...
for epoch = 1:opts.epochs
    ...
    for batch = 1:minibatches
        ...
        for k = 1:N_batchlen
            ...
            for j = 1:outlayer
                [y_hats{j},dy_hats{j}] = perceptron(opts.W{j},xs{j},opts.outactv);
                if j ~= outlayer
                    [y_hats{j},dy_hats{j}] = perceptron(opts.W{j},xs{j},opts.hidactv);
                    % dropout
                  [y_hats{j},dy_hats{j}] = dropout(y_hats{j},dy_hats{j},opts.dropratio);
                    %
                    xs{j+1} = y_hats{j};
                end
            end
    ...
    Yinf = infer(X,opts);
    [~, Yinf] = max(Yinf,[],2); % arg-max
    Ycorr = X(:,opts.L+1:opts.L+opts.P);
    [~, Ycorr] = max(Ycorr,[],2); % arg-max
    c_acc = sum(Yinf==Ycorr);
    c_acc_percent = c_acc*100/opts.D;
    % log training progress
    
    if mod(epoch,500)==0 || (epoch==opts.epochs)
